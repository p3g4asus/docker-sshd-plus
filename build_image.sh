...
docker build --build-arg OPENSSH_VERSION=${OPENSSH_VERSION} --build-arg ALPINE_VERSION=${CURRENT_ALPINE_VERSION} --build-arg SUBVERSION=${SUBVERSION} -t p3g4asus/alpine:latest -t p3g4asus/alpine:${CURRENT_ALPINE_VERSION}.${SUBVERSION} -f- . < Dockerfile